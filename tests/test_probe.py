"""Tests for wyoming_bluetts.probe."""
...
from wyoming_bluetts.probe import probe
...
def test_probe_returns_true_when_data_segment_matches():
    # "bluetts" only appears in the data segment (the TtsProgram name),
    # never in the header line itself -- matches the real server's actual
    # Info response shape.
    response = _build_event_bytes("info", {"tts": [{"name": "bluetts"}]})
    assert b"bluetts" not in response.split(b"\n", 1)[0]
...
def test_probe_returns_false_when_data_segment_does_not_match():
    response = _build_event_bytes("info", {"tts": [{"name": "someone-else"}]})
    port, thread = _start_server(response)
    try:
