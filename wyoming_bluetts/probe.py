"""Minimal local Wyoming Describe/Info probe, used by the Dockerfiles'
HEALTHCHECK and run.sh's discovery-readiness check.
...
newline-terminated, and NOT part of the header line.
"""
...
def probe(port: int, timeout: float = _TIMEOUT_SECONDS) -> bool:
    ...
    try:
        ...
        return False
    return b"bluetts" in data


def main() -> None:
    if len(sys.argv) != 2:
        print("usage: python3 -m wyoming_bluetts.probe <port>", file=sys.stderr)
        sys.exit(2)
    port = int(sys.argv[1])
    if not probe(port):
